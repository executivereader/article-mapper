...
def process_reuters_articles(reuters_articles, client):
    ...
    for article in reuters_articles:
        ...
        output["poster"] = "http://s3.amazonaws.com/mogulinc/listings/image_urls/000/000/447/original/ThomsonReuters.png?1402685587"
        output["movies"] = []
        output["images"] = []
        ...
        if is_a_in_b(terrorism_words,output["topics"]):
            output["priority"] = output["priority"] + 2
        output["priority"] = 3 * output["priority"] / float(article['newsMessage']['itemSet']['newsItem']['contentMeta']['urgency'])
        output["priority"] = adjust_priority_by_time(output["priority"], output["pubDate"])
